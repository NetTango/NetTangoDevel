...
public class Palette {
	...
	/**
	 * Returns a buffered image from the given path  and resized to no more than maxSize width and/or height
	 * give it the NetLogo logo if the image doesn't exist
	 */
	public static BufferedImage createImage(String path, int maxSize) {

		java.net.URL imageURL = Palette.class.getResource(path);
		if(imageURL != null)
		{
		try {
			BufferedImage origImg = javax.imageio.ImageIO.read(imageURL);
					BufferedImage resized = Scalr.resize(origImg, maxSize - 10);
			return Scalr.pad(resized, 5, new Color(0x55ffffff, true));
		...
		{
			try {
				BufferedImage origImg = javax.imageio.ImageIO.read(Palette.class.getResource("/images/NetLogo_logo.png"));
				BufferedImage resized = Scalr.resize(origImg, maxSize - 10);
				return Scalr.pad(resized, 5, new Color(0x33ffffff, true));
			} catch (IOException e) {
	...
	/**
	 * Returns a buffered image from the given path  and resized to no more than maxSize width and/or height
	 * and with a given backgroundcolor
	 * give it the NetLogo logo if the image doesn't exist
	 */
	public static BufferedImage createImage(String path, int maxSize, Color bgc) {
		...
		try {
			BufferedImage origImg = javax.imageio.ImageIO.read(imageURL);
					BufferedImage resized = Scalr.resize(origImg, maxSize - 10);
			return Scalr.pad(resized, 5, bgc);
		} catch (java.io.IOException iox) {
			iox.printStackTrace();
				return null;
			}
